...
const MPESA_CONFIG = {
  ...
  callbackUrl: process.env.CALLBACK_URL || process.env.MPESA_CALLBACK_URL ||
    (NODE_ENV === 'production'
...
app.get('/api/admin/offices', async (req, res) => {
  try {
    if (!db) {
      return res.status(503).json({ error: 'Database not ready' })
    }
...
// POST /api/admin/offices - Create or update office (requires auth)
app.post('/api/admin/offices', checkAuth, async (req, res) => {
  try {
    if (!db) {
      return res.status(503).json({ error: 'Database not ready' })
    }

    const { name, serviceType, username, password } = req.body
    ...
    const existing = await db.select().from(offices).where(eq(offices.username, username)).limit(1).then((rows) => rows[0] || null)

    if (existing) {
      return res.status(409).json({ error: 'Office with this username already exists' })
    }
...
// DELETE /api/admin/offices - Delete office (requires auth)
app.delete('/api/admin/offices', checkAuth, async (req, res) => {
  try {
    if (!db) {
      return res.status(503).json({ error: 'Database not ready' })
    }
...
// PATCH /api/admin/offices - Update office status (requires auth)
app.patch('/api/admin/offices', checkAuth, async (req, res) => {
  try {
    if (!db) {
      return res.status(503).json({ error: 'Database not ready' })
    }
...
// POST /mpesa-express-simulate/ - Handle M-Pesa callback (Render production endpoint)
app.post('/mpesa-express-simulate/', async (req, res) => {
  try {
    if (!db) {
      console.error('🚨 Database not ready for callback')
      return res.status(503).json({ error: 'Service unavailable' })
    ...
    console.log('📞 M-Pesa Callback received at /mpesa-express-simulate/:', JSON.stringify(callbackBody, null, 2))
    ...
    console.error('Error processing M-Pesa callback at /mpesa-express-simulate/:', error)
