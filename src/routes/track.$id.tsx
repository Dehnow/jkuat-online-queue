...
type QueueEntry = {
  ...
  mpesaPaidAt?: string
  claimedAt?: string
  pendingDebts?: Array<{ amount: number; reason: string }>
}
...
export default function TrackPage() {
  ...
  const [processingPhone, setProcessingPhone] = useState('')
  ...
  const handleProcessingConfirm = async () => {
    if (!processingPhone.trim()) {
      setProcessingMessage('Please enter your phone number')
      return
    }

    try {
      setPaymentLoading(true)
      const res = await fetch('/api/queue/process-golden', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          queueId: id,
          action: 'confirm',
          phoneNumber: processingPhone,
        }),
      })
      const data = await res.json()
      ...
      setProcessingMessage('✅ Confirmed! Initiating M-Pesa payment...')
      setTimeout(() => {
        handleInitiatePayment(processingPhone)
        setShowProcessingDialog(false)
      }, 1000)
    } catch (err) {
      setProcessingMessage('Error confirming golden ticket')
      console.error(err)
    } finally {
      setPaymentLoading(false)
    }
  ...
  const handleProcessingCancel = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setPaymentMessage('Cancelled. You can try again anytime.')
      }
    } catch (err) {
      console.error(err)
    } finally {
  ...
  const handleInitiatePayment = async (phone?: string) => {
    const phoneNumber = phone || paymentPhone
    if (!phoneNumber.trim()) {
  ...
  const handleClaimGolden = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPaymentMessage('✅ Golden ticket claimed! You\'ll be served next after the current customers.')
        setTimeout(() => window.location.reload(), 2000)
      } else {
        setPaymentMessage(`Error: ${data.error}`)
      }
    } catch (err) {
      setPaymentMessage('Error claiming ticket')
      console.error(err)
    } finally {
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
          ...
          <div className="space-y-3">
            {entry.status === 'waiting' && (
              <>
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="rounded-xl px-4 py-3 bg-yellow-50 border-2 border-yellow-400">
                      ...
                      {entry.mpesaStatus === 'success' && (
                        <div className="text-xs text-green-700 mt-2">✅ Payment confirmed! Ready to claim your spot.</div>
                      )}
                    </div>
                  </div>
                ) : (
                ...
                {entry.isGolden && entry.mpesaStatus === 'success' && !entry.claimedAt && (
                  <button
                    onClick={() => setShowClaimConfirmDialog(true)}
                    className="w-full mt-3 px-4 py-3 rounded-xl font-bold text-sm text-white transition-all duration-200 hover:shadow-lg"
                ...
                {entry.claimedAt && (
                  <div className="rounded-xl px-4 py-3 mt-2 bg-purple-50 border-2 border-purple-400 text-center">
                    <div className="text-xs font-bold text-purple-900">✨ Golden Ticket Claimed!</div>
                    <div className="text-xs text-purple-800 mt-1">Scheduled for service next</div>
                  </div>
                )}
              </>
            )}
          ...
          <div className="glass rounded-2xl shadow-2xl w-full max-w-md p-7 mt-5 animate-slide-in border-2 border-yellow-400">
            ...
            <div className="space-y-4">
              <div className="bg-yellow-50 rounded-xl p-4 text-sm">
                <div className="font-bold text-yellow-900 mb-2">✨ What is a Golden Ticket?</div>
                <ul className="text-yellow-800 space-y-1.5 text-xs">
                  <li>🕐 <strong>Late to campus?</strong> Don't worry! Upgrade to golden to secure your spot</li>
                  <li>⏰ <strong>When available:</strong> Click CLAIM to re-enter the queue (positioned after 5 people)</li>
                  <li>💳 <strong>Cost:</strong> KES 50 via M-Pesa</li>
                  <li>⚠️ <strong>Important:</strong> If you claim but staff doesn't confirm within the day, KES 2,000 penalty applies</li>
                </ul>
              </div>
        ...
        {showProcessingDialog && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
            <div className="glass rounded-2xl shadow-2xl w-full max-w-md p-7 animate-slide-in">
              <h2 className="text-lg font-bold mb-4 flex items-center gap-2" style={{ color: 'var(--navy)' }}>
                <span>✨</span> Complete Golden Ticket Registration
              </h2>

              <div className="bg-red-50 border-2 border-red-400 rounded-xl p-4 mb-4">
                <div className="text-sm font-bold text-red-900 flex items-center gap-2 mb-1">
                  <span>⚠️</span> Important Warning
                </div>
                <div className="text-xs text-red-800">
                  If you claim this golden ticket but staff doesn't confirm your service within the day, you will be charged a penalty of <strong>KES 2,000</strong> to your account.
                </div>
              </div>

              <div className="mb-4">
                <label className="text-sm font-semibold mb-2 block" style={{ color: 'var(--navy)' }}>
                  Phone Number (M-Pesa):
                </label>
                <input
                  type="tel"
                  placeholder="254712345678 or 0712345678"
                  value={processingPhone}
                  onChange={(e) => setProcessingPhone(e.target.value)}
                  disabled={paymentLoading}
                  className="w-full px-4 py-2 border-2 rounded-lg font-mono text-sm disabled:opacity-50"
                  style={{ borderColor: 'var(--green-dark)' }}
                />
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={paymentLoading || !processingPhone.trim()}
                  className="flex-1 px-4 py-3 rounded-xl font-bold text-sm text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 transition-colors"
                >
                  {paymentLoading ? '⏳ Processing...' : '✅ Successful'}
                </button>
                <button
                  onClick={handleProcessingCancel}
                  disabled={paymentLoading}
                  className="flex-1 px-4 py-3 rounded-xl font-bold text-sm text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 transition-colors"
                >
                  {paymentLoading ? '⏳ Processing...' : '❌ Cancelled'}
                </button>
              </div>
            </div>
          </div>
        ...
        {showClaimConfirmDialog && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
            <div className="glass rounded-2xl shadow-2xl w-full max-w-md p-7 animate-slide-in">
              <h2 className="text-lg font-bold mb-4 flex items-center gap-2" style={{ color: 'var(--navy)' }}>
                <span>👑</span> Claim Golden Ticket
              </h2>

              <div className="bg-purple-50 rounded-xl p-4 mb-4 border-2 border-purple-300">
                <div className="text-sm text-purple-900">
                  <strong>Confirm:</strong> You'll be positioned at queue position <strong>#6</strong> (after 5 people are served). You must show up when called or face a <strong>KES 2,000</strong> penalty.
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-3 rounded-xl font-bold text-sm text-white bg-purple-600 hover:bg-purple-700 disabled:opacity-50 transition-colors"
                >
                  {claimLoading ? '⏳ Claiming...' : 'Yes, Claim Ticket'}
                </button>
                <button
                  onClick={() => setShowClaimConfirmDialog(false)}
                  disabled={claimLoading}
                  className="flex-1 px-4 py-3 rounded-xl font-bold text-sm text-white bg-gray-600 hover:bg-gray-700 disabled:opacity-50 transition-colors"
                >
                  Cancel
                </button>
              </div>
